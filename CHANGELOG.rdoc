= v0.6.4 - EDGE
* enhancements
  * Defined Netzke.isLoading(), useful for testing.
  * Persistence suppor

* API change
  * +endpoint+ DSL call now results in a method called <endpoint_name>_endpoint, _not_ just <endpoint_name> (beware when overriding endpoint definitions)
...
* New: widgets now can check session[:netzke_just_logged_in] and session[:netzke_just_logged_out] automatically set by Netzke after login/logou
...
* Ext.Panel#loadComponent will accept null as url to delete the currently loaded widge
