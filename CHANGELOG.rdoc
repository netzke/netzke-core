...
* API backward incompatibility
  ...
  * mentioning actions in the bbar, tbar, etc, should be explicit, e.g.:
      ...
      :bbar => [:apply.ext_action, :delete.ext_action]
      
  * late_aggregatee option becomes lazy_loading
  * "aggregatees" are now "components"
  * "widgets" are now "components"
  * 'api' is now 'endpoint'
  * persistent_config_enabled? is now persistence_enabled?
  * all actions should now be wrapped up in the js_action method
  * +late_component+ option becomes +lazy_loading+
  * aggregatees are now called components
  * widgets is now called components, too
...
* New
  * Use the +ext+ helper in the views to embed any (pure) Ext component into a view.
...
* Fix: <tt>load_component_with_cache</tt> was throwing exception when the requested component wasn't defined
